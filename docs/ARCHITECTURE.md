...
3. User can create/edit/delete entries through forms.
4. Store flow (`POST /store`) validates input, runs scraper, reads scraped JSON, and creates a `products` row.
5. Update flow (`POST /update/{id}`) validates and updates editable fields.
6. Destroy flow (`POST /destroy/{id}`) removes DB row and related local files.
...
- genre data (`genre`, `genre_english`, `genre_custom`)
