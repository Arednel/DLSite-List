...
- `ProductIndex` reads page size, Index layout, Filter layout, table width, Image Viewer, and work-form modal settings through one batched `ProductIndexSettings` option lookup per render, normalizes filter state once into a local `ProductIndexFilters` value, and derives query/options/sort-icon/return/progress/tag data from that same value without component-level cache invalidation
