...
- `tests/Feature/TagLibraryManagerTest.php`
  ...
  - Parent/child relationships: searchable parent and child plaques, shared help-circle styling and help copy, both relation-edit directions, locked editing tag identity, existing-work backfill with Custom parents, non-destructive relation removal, and cycle rejection
  - Status, filtering, and ordering: compact in-chip hidden-tag indicators; fixed-order staged modal controls; Apply/Clear and invalid-value normalization; All Tags visibility, group, usage, parent/child-role, and own-color filters; duplicate secondary normalization; alphabetical/visible-work-count sorting in both directions with deterministic and configurable ties; search preservation; and unchanged saved order inside Tag Group cards
