...
- `tests/Feature/TagLibraryManagerTest.php`
  ...
  - Status, filtering, and ordering: compact in-chip hidden-tag indicators; fixed-order staged modal controls; Apply/Clear and invalid-value normalization; All Tags visibility, group, usage, parent/child-role, and own-color filters; alphabetical/visible-work-count sorting in both directions with deterministic ties; search preservation; and unchanged saved order inside Tag Group cards
